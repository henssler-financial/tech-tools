---
name: atlas-stacks
description: Interactive skill-stacking concierge. Use when the user knows roughly what they want but not which skills/tools to use, asks "what should I use for X", names several goals at once, or invokes /atlas-stacks. Elicits the goal with AskUserQuestion (one focused round), inventories the skills actually available this session, composes them into an ordered stack (chained Skill invocations with atlas verification riding along), confirms the stack with the user, then executes it stage by stage.
---

# atlas-stacks - compose skills into a stack
...
Never ask what you can discover (stack, framework, existing docs/ state - detect those).
...
atlas family (atlas-engine, atlas-architect, atlas-survey, atlas-cartographer,
atlas-expedition, atlas-orbit, atlas-sextant, atlas-harbor) plus every other installed
...
- **Orchestration rides along.** Any stack with substantive engineering work includes
  `atlas-engine` as the execution stage so subagent discipline, verification, and the
  docs/ gate engage. Setup gaps (no memory, no hooks, no docs/ scaffold) prepend
  `atlas-architect`.
- **Discovery before change.** Mapping/audit skills (atlas-cartographer, atlas-survey,
...
- **Right tool per stage.** UI work stacks design skills before implementation and
  webapp-testing/atlas-expedition after; research questions stack deep-research first;
  recurring work ends by instantiating an atlas-orbit loop.
- Stages that are independent may run as parallel subagent dispatches inside
  atlas-engine; the stack order is about data dependency, not ceremony.
...
definition-of-done: evidence captured, independent verification, docs/ current.
