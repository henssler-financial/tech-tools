...
The orchestrator writes the approved boundary list to .atlas/docs/audits/atlas-audit-<date>/boundaries.md before dispatching Phase 1.
...
The orchestrator collects all charts and writes them to .atlas/docs/audits/atlas-audit-<date>/charts/<feature>.md. The `<feature>` filename must be a filesystem-safe slug (see "Filename safety" below) - never write a raw feature name containing a colon, slash, or space into a path.
...
The orchestrator writes the merged duplication list to .atlas/docs/audits/atlas-audit-<date>/duplications.md.
...
The proposal is written to .atlas/docs/audits/atlas-audit-<date>/proposal.md.
...
All artifacts land under .atlas/docs/audits/atlas-audit-<date>/ as the single source of truth. No loose files in the repo root.
...
The orchestrator writes a short index entry to .atlas/docs/audits/atlas-audit-<date>/index.md listing the run date, feature count, duplication count, and proposal summary (one sentence per merged subsystem).
