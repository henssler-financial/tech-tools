...
How atlas-ux-test relates to the repo-root `webapp-testing` skill, and where
...
| Skill | Location | Role |
|---|---|---|
| atlas-ux-test | `plugins/atlas/skills/atlas-ux-test/` | Orchestrates the full UX test swarm: discovery, persona generation, scripted entry, browser walks, fuzz, calc oracle, synthesis. The single authority for the swarm. |
...
atlas-ux-test orchestrates and verifies. It never navigates the app or enters data
...
```
atlas-ux-test (orchestration + gates)
...
atlas-ux-test supplies: the run directory, the contract snapshot from phase 0, the
...
## What atlas-ux-test passes down
...
## What atlas-ux-test never delegates
