...
2. **Security audit overdue** - has atlas-audit ever run? Is it stale?
3. **Architecture map missing or stale** - has atlas-audit mapped this
   codebase? Is the map older than the last code change?
4. **Run health regressing** - self-telemetry metrics (verifier_coverage,
   inline_ops, unpaired dispatches) trending the wrong way? See
   `atlas-audit references/self-telemetry.md`.
5. **UX coverage gap** - is there a frontend atlas-ux-test has not tested?
6. **Docs drift** - does AGENTS.md match the stack? Is CHANGELOG current?
7. **Recurring task identified** - repeated prompts atlas-loop could
   automate.
8. **Tech debt accumulation** - TODO/FIXME markers, stale branches.
9. **Connector not provisioned** - vendor signals present but the
