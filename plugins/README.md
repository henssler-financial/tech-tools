...
Kimi Code CLI users can browse the custom marketplace with `/plugins marketplace https://github.com/w159/tech-tools/blob/main/.kimi-plugin/marketplace.json` (or pass the local path `.kimi-plugin/marketplace.json` when running from the repo root), then install the plugins you need. You can also install a single plugin directly with `/plugins install https://github.com/w159/tech-tools/tree/main/plugins/<name>`.
